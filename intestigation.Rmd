...
laptop_data_cleaned <- laptop_data_cleaned %>%
  mutate(
    Company = case_when(
      Company == 'Acer' ~ 'Acer',
      Company == 'Asus' ~ 'Asus',
      Company %in% c('Apple', 'Fujitsu', 'Google', 'Huawei', 'LG', 'Microsoft', 'Razer', 'Samsung', 'Xiaomi') ~ 'Others',
      Company == 'Dell' ~ 'Dell',
      Company == 'HP' ~ 'HP',
...
laptop_data_cleaned <- laptop_data_cleaned %>%
  mutate(
    TypeName = case_when(
      TypeName == '2 in 1 Convertible' ~ '2 in 1 Convertible',
      TypeName == 'Gaming' ~ 'Gaming',
      TypeName %in% c('Netbook', 'Workstation') ~ 'Others',
